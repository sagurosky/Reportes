...
public interface StockHistoricoRepository extends JpaRepository<StockHistorico, Long> {
        ...
        /**
         * Stock evolution by ambiente over time - FIXED VERSION
         * Uses subquery to select only the LATEST load for each product/sucursal on a
         * given day
         * to avoid double-counting if multiple files were uploaded.
         */
        @Query(value = """
                        SELECT p.ambiente,
                               sh.fecha_stock,
        ...
        /**
         * Stock evolution by familia (for drill-down) - FIXED VERSION
         */
        @Query(value = """
                        SELECT p.ambiente,
                               p.familia,
